...
def coarseTuning(X, y, k=5):
    ...
    plt.plot(C_range, cv_scores_rbf, 'bo-', linewidth=2)
    plt.title('SVM with rbf kernel')
    plt.xlabel('C')
    plt.xticks(np.logspace(-6, 6, 13))
    plt.ylabel('Accuracy')
    plt.savefig('CoarseTuningParam_rbf.jpg')

    for c in C_range:
        print("\nCross-Validation linear: C=%0.2f\n"%(c))
    ...
    plt.plot(C_range, cv_scores_linear, 'bo-', linewidth=2)
    plt.title('SVM with linear kernel')
    plt.xlabel('C')
    plt.xticks(np.logspace(-6, 6, 13))
    plt.ylabel('Accuracy')
    plt.savefig('CoarseTuningParam_linear.jpg')
    ...
    with open('res_coarse.txt', 'w') as f:
        ...
        for i in range(len(C_range)):
            f.write("rbf: C = %f, acc = %0.2f\n"%(C_range[i], cv_scores_rbf[i]))
        f.write("rbf: Best C = %0.2f\n"%(bestC_rbf))
        f.write("rbf: acc = %f\n"%(bestAcc_rbf))
        for i in range(len(C_range)):
            f.write("linear: C = %f, acc = %0.2f\n"%(C_range[i], cv_scores_linear[i]))
        f.write("linear: Best C = %0.2f\n"%(bestC_linear))
