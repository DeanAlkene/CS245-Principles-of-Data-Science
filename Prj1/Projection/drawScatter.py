...
def draw(suffix):
    ...
    for i, label in enumerate(train_categories):
        plt.scatter(train_2d.loc[train_2d.label==label].x, train_2d.loc[train_2d.label==label].y, s=1, cmap=train_colors[i])
    plt.title('X_train')
    plt.savefig('X_train_scatter_2d_' + suffix + '.jpg')

    plt.figure()
    for i, label in enumerate(test_categories):
        plt.scatter(test_2d.loc[test_2d.label==label].x, test_2d.loc[test_2d.label==label].y, s=20, cmap=test_colors[i])
    plt.title('X_test')
