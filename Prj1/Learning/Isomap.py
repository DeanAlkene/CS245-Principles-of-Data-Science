...
def runIsomap(X_train, X_test, y_train, y_test, comp_range, n_neigh):
    ...
    for n_comp in comp_range:
        print("\nn_comp=%d\n"%(n_comp))
        transformer = Isomap(n_neighbors=n_neigh, n_components=n_comp, n_jobs=8)
    ...
    for i, scores in enumerate([rbf_scores, linear_scores]):
        ...
        with open('res_Isomap_' + kernel + '_' + str(n_neigh) + '.txt', 'w') as f:
...
def draw(comp_range, neigh_range, scoresS, kernel):
    ...
    plt.title('Isomap with SVM ' + kernel + ' kernel')
    plt.xlabel('n_components')
    plt.ylabel('Accuracy')
    plt.savefig('Isomap_' + kernel + '.jpg')
...
def main():
    ...
    for n_neigh in neigh_range:
        ...
        rbf_scores, linear_scores = runIsomap(X_train, X_test, y_train, y_test, comp_range, n_neigh)
