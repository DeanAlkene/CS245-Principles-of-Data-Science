...
def runTSNE(X_train, X_test, y_train, y_test, comp_range, m):
    rbf_scores = []
    linear_scores = []
    for n_comp in comp_range:
        print("\nn_comp=%d\n"%(n_comp))
        transformer = TSNE(n_components=n_comp, perplexity=50.0, method=m)
        transformer.fit(X_train)
        X_train_proj = transformer.transform(X_train)
        X_test_proj = transformer.transform(X_test)
        if n_comp == 2 and m == 'barnes_hut':
            np.save('X_train_proj_2d_TSNE_bh', X_train_proj)
            np.save('X_test_proj_2d_TSNE_bh', X_test_proj)
...
def draw(comp_range, scores, kernel):
    ...
    with open('res_TSNE_' + kernel + '.txt', 'w') as f:
    ...
    plt.title('TSNE with ' + kernel + ' kernel')
    plt.xlabel('n_components')
    plt.ylabel('Accuracy')
    plt.savefig('TSNE_' + kernel + '.jpg')

def main():
    comp_range = [2, 3, 5, 10, 20, 30, 40]
    X_train, X_test, y_train, y_test = loadDataDivided(ifSubDir=True)
    rbf_scores, linear_scores = runTSNE(X_train, X_test, y_train, y_test, comp_range)
