...
def main():
    kernel= 'ITML'
    dim_range = [2, 10, 50, 75, 100, 200, 300]
    k_range = [2, 3, 4, 5, 6, 7, 8, 10, 11, 12, 13, 14, 15, 16, 50, 100, 200, 500]
    ...
    for dim in dim_range:
        ...
        X_train, X_test, y_train, y_test = loadDataDivided(ifSubDir=True, ifScale=False,
                                                           suffix='_' + str(dim) + '_' + kernel)
        KNN.runKNN(X_train, X_test, y_train, y_test, k_range, metric=cosine, metric_params=None,
                   label=str(dim) + '_' + kernel + '_cosine')
